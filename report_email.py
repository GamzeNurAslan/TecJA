...
def get_smtp_settings() -> SmtpSettings:
    ...
    if not host or not from_email:
        raise ReportEmailConfigurationError(
            "Gönderici e-posta hesabı henüz bağlanmamış. Proje "
            "kökünde 'python -m backend.app.configure_email' "
            "komutunu bir kez çalıştırın."
        )

    if username and not password:
        raise ReportEmailConfigurationError(
            "Gönderici hesabının uygulama parolası eksik. "
            "'python -m backend.app.configure_email' komutuyla "
            "e-posta kurulumunu tamamlayın."
        )
